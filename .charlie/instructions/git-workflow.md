...
- `CGW_LOCAL_FILES` and `CGW_LOCAL_FILES_EXEMPT` are read from `.cgw.conf` by the git hooks at run time, so editing the config takes effect on the next commit/push (no `configure.sh` re-run needed); the matcher (`cgw_is_local_file` in `_common.sh`) is the single source of truth — `commit_enhanced.sh`, hooks, and CI all use it
...
- [R6] Scripts that mutate history must call `cgw_create_backup_tag <op>` from `_common.sh` before acting (sets `$CGW_BACKUP_TAG`; op must be in `CGW_BACKUP_OPS`)
