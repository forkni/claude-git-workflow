...
_detect_target_branch() {
  ...
  if [[ -n "${remote_head}" ]]; then echo "${remote_head}"; return 0; fi
  # Check common names
  if git show-ref --verify --quiet refs/heads/main 2>/dev/null; then echo "main"; return 0; fi
  if git show-ref --verify --quiet refs/heads/master 2>/dev/null; then echo "master"; return 0; fi
  echo "main"
}
...
_detect_source_branch() {
  ...
  if [[ -n "${recent}" ]]; then echo "${recent}"; return 0; fi
...
_detect_lint_tool() {
  ...
  if [[ -f "pyproject.toml" ]] || [[ -f "setup.py" ]] || [[ -f "setup.cfg" ]] || [[ -f "requirements.txt" ]]; then
    if command -v ruff &>/dev/null; then echo "ruff"; return 0; fi
    if command -v flake8 &>/dev/null; then echo "flake8"; return 0; fi
    if command -v pylint &>/dev/null; then echo "pylint"; return 0; fi
  fi
  # JavaScript/TypeScript project detection
  if [[ -f "package.json" ]]; then
    if command -v eslint &>/dev/null; then echo "eslint"; return 0; fi
  fi
  # Go project detection
  if [[ -f "go.mod" ]]; then
    if command -v golangci-lint &>/dev/null; then echo "golangci-lint"; return 0; fi
  fi
  # Rust project detection
  if [[ -f "Cargo.toml" ]]; then
    if command -v cargo &>/dev/null; then echo "cargo"; return 0; fi
  fi
