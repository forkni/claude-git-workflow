...
```bash
./scripts/git/fix_lint.sh [--modified-only] [--non-interactive] [--no-venv] [--skip-md-lint] [--md-only]
```

- Fixes code lint (`CGW_LINT_CMD`/`CGW_FORMAT_CMD`) and Markdown (`CGW_MARKDOWNLINT_CMD`, auto-detected at runtime if unset)
- `--skip-md-lint`: code auto-fix only, skip markdown
- `--md-only`: markdown auto-fix only, skip code lint (mutually exclusive with `--skip-md-lint`)
...
| `CGW_MERGE_CONFLICT_STYLE=<merge\|diff3\|zdiff3>` | Set merge conflict-marker style; `merge` is git's two-way default (same as unset), `diff3`/`zdiff3` add the common-ancestor view (`zdiff3` needs git >= 2.35, falls back to `diff3`) |
